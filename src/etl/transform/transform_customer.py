...
def clean_customer(customer_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        raise RuntimeError(f"ERROR: Failed cleaning customer.csv | Missing required column: {e}") from e

    except Exception as e:
        raise RuntimeError(f"ERROR: Failed cleaning customer.csv | Unexpected Error: {e}") from e